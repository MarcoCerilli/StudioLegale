...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $water_sports_gear_url The remote URL.
		 */
		public function __construct( $water_sports_gear_url = '' ) {
			$this->remote_url = $water_sports_gear_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $water_sports_gear_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $water_sports_gear_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $water_sports_gear_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $water_sports_gear_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $water_sports_gear_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $water_sports_gear_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $water_sports_gear_format = 'woff2' ) {
			$this->font_format = $water_sports_gear_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $water_sports_gear_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $water_sports_gear_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $water_sports_gear_url, '/' ) ) {
				$water_sports_gear_parsed_url = parse_url( $this->remote_url );
				return $water_sports_gear_parsed_url['scheme'] . '://' . $water_sports_gear_parsed_url['hostname'] . $water_sports_gear_url;
			}

			return $water_sports_gear_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $water_sports_gear_url    The URL of the remote webfont.
	 * @param string $water_sports_gear_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $water_sports_gear_url, $water_sports_gear_format = 'woff2' ) {
		$water_sports_gear_font = new WPTT_WebFont_Loader( $water_sports_gear_url );
		$water_sports_gear_font->set_font_format( $water_sports_gear_format );
		return $water_sports_gear_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $water_sports_gear_url, $water_sports_gear_format = 'woff2' ) {
		$water_sports_gear_font = new WPTT_WebFont_Loader( $water_sports_gear_url );
		$water_sports_gear_font->set_font_format( $water_sports_gear_format );
		return $water_sports_gear_font->get_url();
	}
}