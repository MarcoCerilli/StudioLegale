...
export default function Navbar({ services }: { services: any[] }) {
  ...
  return (
    <nav className="sticky top-0 z-[999] w-full border-b border-[#c5a5a5]/10 bg-[#4a3434] text-white">
      <div className="flex justify-between items-center h-24 md:h-28 px-6 md:px-12 max-w-7xl mx-auto relative">
        ...
        <Link href="/" className="flex items-center gap-4 group z-[1001]" onClick={closeMenu}>
          ...
          <div className="flex flex-col border-l border-white/30 pl-4 md:pl-6">
            <span className="font-serif font-bold text-base md:text-lg tracking-[0.1em] leading-tight uppercase text-white">
              Studio Legale
            </span>
            <span className="font-serif text-[10px] md:text-xs tracking-[0.5em] text-white/70 uppercase">
              Fusco
            </span>
          </div>
        </Link>
        ...
        {/* HAMBURGER */}
        <button
          className="md:hidden relative z-[1001] w-12 h-12 flex items-center justify-center text-white focus:outline-none"
        ...
        <div className={`
          ...
          bg-[#4a3434]/98 md:bg-transparent backdrop-blur-xl md:backdrop-blur-none
        ...
        `}>
          ...
          <div className="md:hidden mt-32 mb-4 w-full text-center border-b border-white/20 pb-4">
            <p className="font-serif text-white tracking-[0.4em] text-[10px] opacity-60">NAVIGAZIONE</p>
          </div>
          ...
          <div className="relative group flex flex-col items-center md:block">
            ...
            <div className="hidden md:group-hover:block absolute top-full right-0 pt-4 w-64 translate-y-0">
              <div className="bg-[#2d2424] border border-[#c5a5a5]/20 shadow-2xl overflow-hidden rounded-sm">
                {services?.map((s) => (
                  <Link
                    key={s.slug}
                    href={`/aree-di-attivita/${s.slug}`}
                    className="block px-6 py-4 text-[9px] text-white hover:bg-[#8b5e5e] border-b border-[#c5a5a5]/5 last:border-0 transition-all duration-300"
                    onClick={closeMenu}
                  >
                    {s.title}
                  </Link>
